...
- (void)nativeAdsLoaded:(nullable NSArray *)nativeAds {
    ...
    if (self.adTypes.count == 1) {
        if ([self.adTypes containsObject:kGADAdLoaderAdTypeNativeAppInstall]) {
            isInstalled = YES;
        }
        if ([self.adTypes containsObject:kGADAdLoaderAdTypeNativeContent]) {
            isInstalled = NO;
        }
    }
    
    if (isInstalled) {
        MintegralMediatedNativeAppInstallAd *mediatedAd =
        [[MintegralMediatedNativeAppInstallAd alloc] initAppInstallAdWithNativeManager:self.mtgNativeAdManager mtgCampaign:campaign withUnitId:self.localNativeUnitId videoSupport:self.video_enabled];
        [self.delegate customEventNativeAd:self didReceiveMediatedNativeAd:mediatedAd];
    }else{
        MintegralMediatedNativeContentAd *mediatedAd = [[MintegralMediatedNativeContentAd alloc] initContentAdWithNativeManager:self.mtgNativeAdManager mtgCampaign:campaign withUnitId:self.localNativeUnitId videoSupport:self.video_enabled];
        [self.delegate customEventNativeAd:self didReceiveMediatedNativeAd:mediatedAd];
    }
}
